...
class DiskDataset(Dataset):
    def __init__(self, in_root='/home/geyuan/code/mdt24rss_fork/dataset/litdata',
        ...
        self.ori_path = os.path.join(in_root, split)
...
def diskdata_extract(
    ...
    # 1. Read data
    in_abs = os.path.join(root, split)
    if not os.path.exists(in_abs):
        print(f'[diskdata_extract][Warning] {in_abs} does not exist, skipping!')
        return
    os.makedirs(out_dir, exist_ok=True)
    extracted_save_path = os.path.join(root, split, 'extracted', f"ep_{extract_key}.npy")
    if "ep_rel_action.npy" in extracted_save_path:  # missing `s` bug by yangming
        extracted_save_path = extracted_save_path.replace('ep_rel_action.npy', 'ep_rel_actions.npy')
    ...
    disk_dataset = DiskDataset(in_root=root, split=split, target_key=extract_key)
...
if __name__ == '__main__':
    ...
    Params:
        ...
        out_dir: if None, output will be saved to {in_root}/{in_split}/extracted/ep_{extract_key}.npy
        force: whether to overwrite existing extracted data
    '''
    args = argparse.ArgumentParser()
    args.add_argument('-i', '--in_root', type=str, default='/home2/mingyang/projs/LIBERO/calvin_format',
                      help='This path contains training/ and validation/ directories.')
    ...
    args.add_argument('-o', '--out_dir', type=str, default=None,
                      help='The extracted *.npy will be saved under this directory. '
                           ...
                           'as {in_root}/{in_split}/extracted/ep_{extract_key}.npy .'
                           'Otherwise, the saving dir will '
                           'be {out_dir}/{in_split}/extracted}')
    ...
    for split in splits:
        if opts.out_dir is None:
            out_dir = str(os.path.join(opts.in_root, split, 'extracted'))
            # i.e {in_root}/{in_split}/extracted/
        else:
            out_dir = str(os.path.join(opts.out_dir, split, 'extracted'))
            # i.e {out_dir}/{in_split}/extracted/
        diskdata_extract(opts.in_root, split,
                         opts.extract_key,
                         out_dir,
